# イヤホン探し (EarbudFinder)

Bluetoothイヤホンの**接続が切れた瞬間の位置(GPS)を自動で記録**して、後から地図で探せるようにするiOSアプリです。

イヤホンケースには AirTag を付けられますが、**イヤホン本体には付けられません**。ポケットに入れたまま無くす…そんなときに「最後にBluetoothが切れた場所」が分かれば見つけやすくなる、という発想で作りました。

## 仕組み

iOSでは、AirPodsのようなオーディオ用Bluetooth機器は CoreBluetooth では直接扱えません。そこで本アプリは **`AVAudioSession` のオーディオ経路（ルート）変更通知** を使って、Bluetoothが切れた瞬間（出力経路から機器が消える＝`oldDeviceUnavailable`）を検知します。検知すると、その時点の GPS 位置を記録します。

> **「音が止まったら」ではなく「Bluetoothが切れたら」記録します。** 自動耳検出（耳から外すと一時停止）はBluetooth接続を維持するため、原則として記録されません。記録されるのは、置き忘れて歩き去る（圏外）・ケースにしまう・電源が切れる、など実際に接続が切れたときです。

```
Bluetooth接続中 → 接続が切れる → ルート変更通知を受信 → （数秒の確認猶予）→ 現在地を保存 + 通知
```

### 誤検知を抑える確認猶予（デバウンス）
切断候補を検知しても即記録せず、数秒（既定6秒）待ちます。その間に同じ機器が再接続したら記録をキャンセルします。これにより、一瞬の経路の揺れや、別のオーディオ機器へ切り替えた際の誤記録を抑えます。

### バックグラウンドでも検知するための工夫
他アプリ（Apple Music / Spotify など）で音楽を聴いている最中にイヤホンが切れても捕まえられるよう、アプリは「音量ゼロの無音」を `.mixWithOthers` でループ再生し、自分のオーディオセッションをアクティブに保ちます（`KeepAliveAudio`）。これにより、アプリがバックグラウンドにいてもルート変更通知を受け取れます。設定でON/OFFできます（電池を消費するため）。

> ⚠️ この「無音再生によるキープアライブ」は個人利用では有効な手法ですが、App Store のガイドライン上はグレーゾーンです。ストア配布を考える場合は注意してください。

## 主な機能
- 接続が切れた場所を**座標(GPS)で自動記録**（端末ローカルに保存）
- 最後の場所を**アプリ内の Google マップ**で表示（タップで全画面）
- 履歴は**日付ごと（今日・昨日・日付）にグループ表示**して見返せる
- 各記録に緯度・経度を表示。タップでアプリ内 Google マップを全画面表示
- 切断時にローカル通知
- 切断履歴の削除・全消去
- 「いまの位置でテスト記録」ボタン（動作確認用）

### アプリ内 Google マップについて
外部の Google マップアプリを開くのではなく、`WKWebView` に Google マップの埋め込み（APIキー不要の `output=embed`）を読み込み、**アプリ内で**地図を表示します（`GoogleMapView`）。表示には通信が必要です。

## 構成
| ファイル | 役割 |
|---|---|
| `EarbudFinderApp.swift` | アプリのエントリポイント |
| `AppModel.swift` | 全体の取りまとめ（許可リクエスト・監視開始・記録） |
| `AudioRouteMonitor.swift` | オーディオ経路を監視し切断を検知 |
| `LocationManager.swift` | 位置情報の取得（常時許可・バックグラウンド更新） |
| `KeepAliveAudio.swift` | バックグラウンド検知用の無音再生 |
| `DisconnectStore.swift` | 切断イベントのJSON永続化 |
| `DisconnectEvent.swift` | 1件の切断イベントのモデル |
| `NotificationManager.swift` | ローカル通知 |
| `GoogleMapView.swift` | アプリ内 Google マップ（WKWebView 埋め込み）と全画面表示 |
| `ContentView.swift` | 画面（地図・日付別履歴・設定） |

## ビルド方法
1. macOS + Xcode 15 以降
2. `EarbudFinder/EarbudFinder.xcodeproj` を Xcode で開く
3. 署名チーム（Signing & Capabilities）を自分の Apple ID に設定
   - `PRODUCT_BUNDLE_IDENTIFIER` は `com.example.EarbudFinder` なので、必要に応じて自分のものに変更
4. **実機（iPhone）** を接続して Run
   - 位置情報・通知・バックグラウンドオーディオは実機での確認を推奨

### 使う権限（Info.plist 設定済み）
- 位置情報（常に許可を推奨）: `NSLocationAlwaysAndWhenInUseUsageDescription` ほか
- Background Modes: `audio`, `location`
- 通知（コードから実行時に要求）

## 動作確認の手順（実機）
1. アプリを起動し、位置情報を「常に許可」、通知を許可
2. Bluetoothイヤホンを接続（ステータスにデバイス名が出る）
3. イヤホンの電源を切る／ケースにしまう／十分離れる → 切断
4. 通知が来て、アプリに「最後に切れた場所」が地図表示される

> シミュレータでは Bluetooth ルート変更を再現しにくいため、「テスト記録」ボタンで保存・地図表示・マップ連携の動作確認ができます。

## シナリオ別の挙動
- **置き忘れて歩き去った（圏外で切断）**: 約10m離れた地点で切断 → そのときの現在地（≒イヤホンを置いた場所）を記録。一番役立つケースです。
- **外したまま放置 → 電源が切れた**: 近くにいるうちに電源が切れれば、その場所を記録。すでに歩き去って圏外切断が先に起きていれば、その「置いた場所」が記録済みです（圏外切断が最初に発生するため）。
- **耳から外しただけ（耳検出ON）**: Bluetoothは接続維持なので記録されません。

## 既知の制限
- 完全に圏外になる前に電源が切れ、かつ自分も離れた場所にいる場合、記録される位置は「電源が切れた瞬間の自分の現在地」になり、イヤホンの場所とはズレます（イヤホン本体が自己位置を発信しない以上、原理的な限界）。
- 耳検出をOFFにしている場合など、機種・設定によっては「耳から外す」操作が誤記録につながる可能性があります（シンプル方式のトレードオフ）。
- 別のBluetooth/オーディオ機器へ手動で切り替えた直後など、ごく稀に誤記録の可能性があります（確認猶予で大半は抑制）。
- 端末が完全に電源オフ・アプリが強制終了・OSにkillされた状態では検知できません。
- 位置精度は GPS 環境に依存します（屋内・地下などは粗くなります）。
